...
function MprisService(params) {
  ...
  this._exportRootInterface(params.root);
  this._exportPlayerInterface(params.player);
}
/**
  TODO: Continue docs...
...
MprisService.createService = function (params, cb) {
  ...
  // Request our name
  mprisService.requestName(params.name, function handleResponse (err, code) {
    // Callback with error, code, and our service
    cb(err, code, mprisService);
  });
};
MprisService.prototype = {
  requestName: function (name, cb) {
    this.dbus.requestName(name, cb);
  },

  _exportRootInterface: function (rootParams) {
